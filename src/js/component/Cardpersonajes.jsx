...
export const Cardpersonaje = () => {
  ...
  const handleFavorite = (item) => {
    ...
    if (Array.isArray(store.ArrayFavoritos)) {
      actions.agregarFavoritos(item, "personajes"); // Pasamos el tipo correcto
    } else {
  ...
  useEffect(() => {
    actions.ArrPersonajes();
  }, [actions]); // Dependencia corregida

  return (
    <div className="tabla d-flex justify-center mt-1">
      <table className="table">
        ...
        <tbody>
          {store.ArrayPersonajes?.map((item, index) => (
            <tr key={index}>
              <td>
              ...
              <td>
                ...
                <button className="btn btn-ghost">
                  <Link className="text" to={`/vistas/${item.uid}`}>details</Link>
                </button>
