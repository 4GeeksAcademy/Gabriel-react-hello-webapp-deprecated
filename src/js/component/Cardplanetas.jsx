...
export const Cardplanetas = () => {
  ...
  const handleFavorite = (item) => {
    ...
    if (Array.isArray(store.ArrayFavoritos)) {
      actions.agregarFavoritos(item, "planetas"); // Pasamos el tipo correcto
    } else {
  ...
  return (
    <div className="tabla d-flex justify-center mt-4">
      <table className="table">
        ...
        <tbody>
          {store.ArrayPlanetas?.map((item, index) => (
            <tr key={index}>
              <td>
              ...
              <td>
                ...
                <button className="btn btn-ghost">
                  <Link className="text" to={`/vistasplanetas/${item.uid}`}>details</Link>
                </button>
